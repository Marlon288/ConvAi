...
export class CloseVectorHNSWNode extends CloseVectorSaveableVectorStore {
    ...
    constructor(embeddings: CloseVectorEmbeddings, args: CloseVectorHNSWLibArgs<HierarchicalNSWT> & { accessKey?: string; secret?: string; }) {
        super(embeddings);
    ...
    async saveToCloud(options: Omit<Parameters<typeof upsertIndex>[1], 'accessKey' | 'secret'>) {
        ...
        const uuid = await upsertIndex(this, _options);
        this.uuid = uuid;
        return uuid;
    }
    ...
    static async loadFromCloud(options: Parameters<typeof getIndex>[0]) {
        const { accessKey, secret } = options;
        if (!accessKey || !secret) {
            throw new Error('No accessKey or secret set');
        }

        const instance = await getIndex(options);
        instance.accessKey = accessKey;
        instance.secret = secret;
        instance._uuid = options.uuid;
        return instance;
    }
